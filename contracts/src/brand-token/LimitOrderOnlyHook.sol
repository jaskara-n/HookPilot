...
/// @title LimitOrderOnlyHook
/// @notice Onchain limit order hook for Uniswap v4 pools.
contract LimitOrderOnlyHook is BaseHook {
    ...
    constructor(IPoolManager _manager, IERC20 _stablecoin) BaseHook(_manager) {
        if (address(_stablecoin) == address(0)) revert AddressCannotBeZero();
